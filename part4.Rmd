...
plot(pool_pa_change~PrevS, data = results,ylab="pool_pa_D",xlab="logit(Prevalence)")
...
plot(pool_abu_change~PrevS, data = results,ylab="pool_abu_D",xlab="logit(Prevalence)")
...
Testataan muuntaa Prev vaikkapa log-muunnoksella. Tämä tehty jo koodin alussa. Muuttuja PrevS. Muunnosta voi käydä vaihtamassa koodin alussa.
...
#luottamusvälit
confint(model.avg(output1, subset=delta<4))
```
...
Ainakin prevalenssin residuaalit näyttävät ihan ok:lta. 
...
```{r }
#sovitetaan malli uudestaan REML asetuksilla
malli8 <- gls(pool_abu_change ~ logmass+PrevS, data = results,correlation=bm.birds2,method="REML",na.action="na.omit")
summary(malli8)
```
