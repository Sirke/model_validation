...
## Aineisto sisään ja taulukoiden kokoaminen
...
Vertailun vuoksi teen ensin analyysit huomioimatta fylogeniaa ja sen jälkeen fylogenia huomioiden.  
...
Malli0 näyttäisi olevan näistä paras. Mikään ei siis selitä ennustustarkkuutta.
...
Malli 3 ja 8 ovat melkein yhtä hyviä. Malli 3 on yksinkertaisempi. Sen mukaan paino olisi selittävä tekijä.
...
Luettiinko model averaging taulukosta full average-arvoja? Sen mukaan mikään muuttuja (ei edes paino) ei selitä ennustustarkkuutta.  

The ‘subset’ (or ‘conditional’) average only averages over the models where the parameter appears. An alternative, the ‘full’ average assumes that a variable is included in every model, but in some models the corresponding coefficient (and its respective variance) is set to zero. Unlike the ‘subset average’, it does not have a tendency of biasing the value away from zero. The ‘full’ average is a type of shrinkage estimator and for variables with a weak relationship to the response they are smaller than ‘subset’ estimators. Lisää voi lukea [täältä](https://theoreticalecology.wordpress.com/2018/05/14/model-averaging-in-ecology-a-review-of-bayesian-information-theoretic-and-tactical-approaches-for-predictive-inference/).
...
Paino näyttäisi olevan merkitsevä, mutta rajoilla.
...
Malli13 paras. Kaikki paitsi Hab merkitseviä muuttujia. Yksinkertaisempi malli9 ei kuitenkaan kaukana.
...
Prevalenssi enää merkitsevä jos tehdään model averaging?
...
Eli, selittääkö vain prevalenssi ennustustarkkuutta?
...
Malli4 on paras mutta malli0 on melkeinpä yhtä hyvä ja yksinkertaisempi. 
...
Mikään ei taida selittää ennustustarkkuutta.
