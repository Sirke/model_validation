...
En tiedä, onko mitään järkeä tuijotella nollamallin residuaaleja mutta siinä ne nyt ovat. Ei kai noissa mitään hälyyttäviä kuvioita ole.
Eli pa-aineistolla mikään lajiominaisuus ei selitä ennustustarkkuutta.


#### Vai pitäisikö method olla "REML"? 

Tässä [esimerkissä](https://lukejharmon.github.io/ilhabela/instruction/2015/07/03/PGLS/) ja Aksun esimerkkikoodissa metodi on "ML" mutta Otson luentoesimerkissä metodi on "REML". Otson esimerkissä ei tehdä mallin fixed-osan valintaa. Yritän selvittää mistä on kyse ja kumpi metodi sopii tähän.

Otson koodissa fylogenia rakennetaan "corPagel" asetuksilla. Tällä ei kai kuitenkaan ole suurta eroa corBrownian-asetukseen.

Kokeilen koko analyysin uudestaan REML asetuksilla:
...
REML ei aiheuta muutosta tuloksiin. Nollamalli on nyt entistä selvemmällä erolla parhain malli.
...
Residuaalit eivät ole tasaisesti jakautuneita, etenkään prevalenssilla. Ei hyvä. 


Kokeilen mallin valintaa uudestaan "REML" asetuksella:
```{r }
#luodaan eri mallivaihtoehtoja
...
Nyt kärkeen menevät vain malli 4 ja 8. Näistä 4 on yksinkertaisempi eli vain prevalenssi olisi merkitsevä.
...
Residuaalit ehkä entistä enemmän vinksallaan. Harvinaisemmilla lajeilla vaihtelu ennustustarkkuudessa on suurta. Pitäisikö mallissa huomioida tämä erilainen vaihtelu jotenkin?


#### Testailua erilaisilla varianssirakenteilla

Kokeilen erilaisia varianssirakennevaihtoehtoja:  
Tässä asetus pitää olla REML.
```{r}
f1=formula(pool_abu_change ~ Mig + Hab + logmass + Prev) #tässä pitäisi olla mukana kaikki mahdolliset interaktiotkin mutta malli ei silloin toimi

m.gls1 <- gls(f1, data = results, correlation=bm.birds2, method="REML", na.action="na.omit")
summary(m.gls1)
...
m.gls2 <- gls(f1, weights=vf2, data = results, correlation=bm.birds2, method="REML", na.action="na.omit")

vf3<-varExp(form=~logmass)
m.gls3 <- gls(f1, weights=vf3, data = results, correlation=bm.birds2, method="REML", na.action="na.omit")

vf4<-varIdent(form=~1|Hab)
m.gls4 <- gls(f1, weights=vf4, data = results, correlation=bm.birds2, method="REML", na.action="na.omit")

vf5<-varIdent(form=~1|Mig)
m.gls5 <- gls(f1, weights=vf5, data = results, correlation=bm.birds2, method="REML", na.action="na.omit")

vf6<-varIdent(form=~1|Mig*Hab)
m.gls6 <- gls(f1, weights=vf6, data = results, correlation=bm.birds2, method="REML", na.action="na.omit")

vf7<-varComb(varIdent(form=~1|Mig),varExp(form=~Prev))
m.gls7 <- gls(f1, weights=vf7, data = results, correlation=bm.birds2, method="REML", na.action="na.omit")
...
par(op)
```
Nämä residuaalit näyttävät ihan ok:lta(?)
...
Nämä residuaalit näyttävät mielestäni paremmilta kuin alkuperäiset. 