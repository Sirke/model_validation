...
Vai pitäisikö method olla "REML"? Kokeilen koko analyysin uudestaan.

Tässä [esimerkissä](https://lukejharmon.github.io/ilhabela/instruction/2015/07/03/PGLS/) ja Aksun esimerkkikoodissa metodi on "ML" mutta Otson luentoesimerkissä metodi on "REML". Yritän selvittää mistä on kyse ja kumpi metodi sopii tähän.
...
Kokeilen tämänkin uudestaan "REML" asetuksella:
```{r }
#luodaan eri mallivaihtoehtoja
...
Nyt kärkeen menevät vain malli 4 ja 8. Näistä 4 on yksinkertaisempi ja vain prevalenssi on merkitsevä.
...
Residuaalit entistä enemmän vinksallaan. Harvinaisilla lajeilla vaihtelu ennustustarkkuudessa on suurta. Pitäisikö mallissa huomioida tämä erilainen varianssi jotenkin?


Kokeilen lisätä prevalenssille varianssia:
```{r}
...
Kokeilen lisätä massalle varianssia:
```{r}
...
Vielä voisi lisätä varianssia habitaatille tai muuttokäyttäytymiselle. Mutta niiden kohdalla validointi-kuvaajat eivät näytä niin pahoilta, niin jätän ne niin kuin ne on.

Mallin valintaa kun prevalenssille suodaan vaihtelua:  
En tiedä pitäisikö asetus olla "ML" vai "REML".
```{r }
#luodaan eri mallivaihtoehtoja
...
Olisko nämä hieman paremman näköiset residuaalien jakaumat?
...
Edelleen Mig on ainoa merkitsevä tekijä. Hmm..
