...
Mallinvalinnasta AIC-arvojen avulla voi lukea [täältä](http://webhome.auburn.edu/~tds0009/PDFs/arnold%20et%20al.%202010.pdf). Tosin tämän artikkelin mukaan olisin ehkä ennemmin vertaillut weights-arvoja. Lainaus: "If the primary objective of modeling is to evaluate the relative importance of many potential predictor variables, such as in many habitatselection
...
Paino ja prevalenssi merkitseviä full-average vertailussa. Eli yksinkertaisin malli8 näyttäisi sisältävän tarvittavat selittävät tekijät. 
...
```{r}
#malli uudestaan
model <- gls(pool_abu_change ~ log(Mass)+log(Prev), data = results,correlation=bm.birds2,method="REML",na.action="na.omit")
...
visreg(model, "Mass",ylab="Prediction accuracy",xlab="Mass") #,gg=TRUE)+theme_bw() + geom_smooth(method="loess", col='#FF4E37', fill='#FF4E37')
```

Varsinkaan painon visualisointi ei oikein vakuuta.
