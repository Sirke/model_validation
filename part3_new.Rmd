...
Ay1 <- st_as_sf(Ay1, coords = c("x", "y"), crs = 3035)
st_crs(Ay1)

Ay2 <- st_as_sf(Ay2, coords = c("x", "y"), crs = 3035)
st_crs(Ay2)

Ap1 <- st_as_sf(Ap1, coords = c("x", "y"), crs = 3035)
st_crs(Ap1)

Ap2 <- st_as_sf(Ap2, coords = c("x", "y"), crs = 3035)
st_crs(Ap2)
```
...
grid=st_read("C:/Users/U1701911/OneDrive - University of Helsinki/github_mallinnus/model_validation/gridit/grid50km.shp")
head(grid,n=4)
plot(st_geometry(grid))
grid=st_set_crs(grid,3035)
```
...
pts.poly <- point.in.poly(Ay1, grid,sp=F)
...
pts.poly <- point.in.poly(Ay2, grid,sp=F)
data<-as.data.frame(pts.poly)
data%>%
  group_by(grid_id) %>%
...
pts.poly <- point.in.poly(Ap1, grid,sp=F)
data<-as.data.frame(pts.poly)
data%>%
  group_by(grid_id) %>%
  summarise_at(vars(-Route), list(~mean(., na.rm=T))) %>%
...
pts.poly <- point.in.poly(Ap2, grid,sp=F)
data<-as.data.frame(pts.poly)
data%>%
  group_by(grid_id) %>%
  summarise_at(vars(-Route), list(~mean(., na.rm=T))) %>%
...
names(paY1)
drops <- c("routeid","left","top","right","bottom","geometry")
...
M5<- MCMCglmm(accuracy ~ 1, random=~species, data = poolresPA,family="gaussian",  ginverse=list(species=bm.birds),nitt=1003000, burnin=3000, thin=1000)
```
...
### 3.3.4 Parhaan mallin validointi kuvaajista

```{r}
plot(M3$Sol, auto.layout=F)
```

### 3.3.5 Pa-aineisto lme-fuktiolla

Sama mutta lme-fuktiolla, ilman fylogeniaa.
...
A5=lme(accuracy~1,data=poolresPA,random=~1|species,method="ML",na.action="na.exclude")
...
### 3.3.6 Analyysi abu-aineistolla
...
N5<- MCMCglmm(accuracy ~ 1, random=~species, data = poolresABU,family="gaussian",  ginverse=list(species=bm.birds),nitt=1003000, burnin=3000, thin=1000)
```

```{r}
output2=model.sel(N1,N2,N3,N4,N5)
output2
```

### 3.3.6 abu-aineisto, parhaan mallin validointi kuvaajista

```{r}
summary(N1)
plot(N1$Sol, auto.layout=F)
```

### 3.3.7 Abu-aineisto, analyysi lme-funktiolla

Sama mutta lme-funktiolla eli ilman fylogeniaa.
...
A5=lme(accuracy~1,data=poolresABU,random=~1|species,method="ML",na.action="na.exclude")
...
### 3.3.8 Kuvaaja luottamusväleillä
