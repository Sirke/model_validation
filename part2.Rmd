...
Mietin, olisiko tässäkin kohtaa pitänyt ottaa lajien välinen fylogenia huomioon?
...
Testailen taulukon tekemistä stargazer-paketilla:

```{r}
library(stargazer)
stargazer(A2full, title="Results",type="text", align=TRUE,dep.var.labels="Prediction accuracy",covariate.labels=c("method B","method C", "method D","datatype POC", "method B : datatype POC","method C: datatype POC", "method D : data type POC"))
```
...
Pa-aineistolla vain C ja D eroavat toisistaan. Hmm? Aiemmin emmeans-funktiolla laskin, että pa-aineistolla A/B ja C/D vertailuissa eroja ei löytynyt. Kumpi on oikein?
...
Simple main effect of data type (pa) was significant for all other methods except for C and D.
...
#    scale_fill_discrete(name = "Data type", labels = c("abu", "POC"))+
