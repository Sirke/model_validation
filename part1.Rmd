...
```
# Ennustustarkkuuksien laskeminen
...
Yhtä lajia tarkastellessa huomaa miten pa-aineistolla ennusteet ovat lähellä nollaa mutta havainnot vaihtelevat "rajummin" -1 ja 1 välillä. Ennusteet ovat positiivisia mutta havainnot taitavat olla suuremmaksi osaksi nollaa tai negatiivisia. Viherpeipon ennustustarkkuus olikin pa-aineistolle -0.12.
...
Plottaan testiksi yhden reitin lajikohtaiset ennustetut ja todelliset muutokset.
...
Yksi piste on siis yhden lajin ennustettu ja havaittu muutos tällä reitillä. 
...
```{r}
#plotataan kaikki reitit kartalle
#luetaan sisään reitit, havainnot ja ennusteet
#coordinates(selroutes_pa)<-c("x","y")
#proj4string(selroutes_pa) <-CRS("+init=EPSG:3035")


#maiden rajat
#rajat<- readOGR(dsn="./gridit", layer="NOFISWE_borders")
#proj4string(rajat)
#plot(rajat)
#plot(selroutes_pa,add=T,pch=16)

#jatkuu...

```
Homma kesken. Yritän työstää tällaisia karttoja R:llä:

```{r echo=F, out.width='60%'}
knitr::include_graphics('./pa_route_accuracies.jpg')
knitr::include_graphics('./abu_route_accuracies.jpg')
```
...
Plottaan malliksi taas viherpeipon.
...
Muista, että arvot ovat yksilömäärien logaritmeja. 
...
Plottaan taas testiksi yhden reitin:
