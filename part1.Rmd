...
## 1.1 Ennustemallit
Malli on nyt ajettu 120 lajilla, joiden prevalenssi on ollut vähintään 0.05. Tällä hetkellä saatavilla on ollut vasta thin=100 aineisto. Mallista on ajettu kahta versiota. Toisessa on käytetty aineistoa vain vuoteen 1999 asti ja toisessa vuoteen 2016 asti.

Malleja, joiden tekemisessä oli käytetty aineistoa vain vuoteen 1999 asti, pyydettiin ennustamaan tulevaisuutta.

Malli: XFormula = ~ JunJul + DJF + AprMay  + Urb + Br + Co + Op + Ma + We + Effort*Method  
...
XFormula = ~ JunJul + DJF + AprMay+
  Urb + Br + Co + Op + Ma + We + Effort*Method
for (thin in c(1)){
  samples = 1000
  nChains = 4
...
**Huom!** Method ja Effort ovat nyt kumpiakin samassa muuttujassa sekaisin. Eli Method voi olla P tai L (point tai line) ja Effort voi olla esim. 20 (pistettä) tai 7000 (metriä). Jossain vaiheessa aineistossa oli muistaakseni dummy-muuttuja Method, joka oli linjalle 1 ja pisteille 0. Effort oli jaettu kahteen muuttujaan Effort-line (metreissä) ja Effort-point (pisteiden lkm), joista toinen jäi siis tyhjäksi riippuen oliko kyseessä pistelaskenta tai linjalaskenta.

Muistutuksena, mitä lähtöaineisto sisältää:
```{r}
...
Molemmat metodit ennustavat hyvin. A:lle ennusteet (eli interpoloidessa) ovat vähän parempia kuin B:lle. 
...
### 1.3.5 Muutama esimerkkilaji

Plottaan parille lajille ennustetut (D) ja todelliset muutokset reiteittäin. Yksi piste on siis yksi reitti.
...
Korrelaatiot on laskettu useiden reittiparien avulla pa-aineistossa. Hyvä niin.


### 1.3.6 Tulokset

Pa-aineiston kunkin metodin ennustustarkkuuksien keskiarvot:
```{r}
#create a table
...
### 1.3.7 Todelliset ja ennustetut muutokset tutkimusalueella

Tarkastellaan absoluuttisia todellisia ja ennustettuja muutoksia esiintymistodennäköisyyksissä koko tutkimusalueella. Ennusteet on tehty joko tyyliin C tai D, eli koko aineistoa tai osittaista aineistoa hyödyntämällä. Paperiin voisi tarvittaessa varmaan valita vain metodin D.
...
#### tapahtuneen muutoksen ja ennustustarkkuuden suhde

Kuvaaja muutoksen suuruuden ja ennustustarkkuuden suhteesta:
...
Eli tästä pitäisi voida päätellä, että jos muutosta on tapahtunut, suuntaan tai toiseen, runsastumista ja vähentymistä, ennustustarkkuus pitäisi olla suurempi, kuin jos lajin esiintymisessä ei ole tapahtunut mitään muutosta, jolloin on vaikeampi mallintaa taikka ennustaakaan mitään. Smootherin pitäisi olla lavean U:n muotoinen paraabeli, jonka pohja on origossa tai sen alapuolella?

Näistä kuvaajista on ehkäpä vaikea saada paljoa irti koska ennustustarkkuudet ovat niin huonoja yleisesti. Ehkä myös siksi, että muutos on laskettu koko alueelle. Eli vähentyminen toisaalla ja runsastuminen toisaalla saattavat kumota toisensa.

Lasken varianssin muutoksista jokaisella 264 linjalla, jota ennustustarkkuuden laskemiseen on käytetty.
...
Ei varsinaista selvää suhdetta muutoksen varianssin ja ennustustarkkuuden välillä. Ennemminkin paljon lajeja, joilla on pieniä muutoksia, mutta ennustustarkkuus voi olla mitä tahansa, suht hyväkin vaikka muutokset eivät olisikaan olleet suuria.

Onko tuo varianssi paras tapa mitata sitä, onko muutosta tapahtunut paljon vai vähän? On kai.
...
Lasken kuinka monesta havaintoparista ennustustarkkuus kullekin lajille lasketaan.
...
**HUOM!** Abundance aineistolla riittää, että lajia on kunkin periodin aikana havaittu edes kerran. PA-aineistolla havaintoja piti olla vähintään kaksi. Kummallakin aineistolla reitillä pitää olla käyty vähintään kaksi kertaa. 
...
**Viherpeippo:** Abu-aineistolla viherpeipon muutoksen ennusteet ovat suurimmaksi osaksi hieman negatiivisia ja havainnot (joissa taas suurta vaihtelua) ovat myös suurimmaksi osaksi lähellä nollaa tai negatiivisia. Viherpeipon ennustustarkkuus onkin abu-aineistolla (0.016) hieman parempi kuin pa-aineistolla (-0.12). 

**Pohjansirkku:** Pohjansirkun ennustustarkkuus oli 0.95. Lasken kuinka moneen havaintopariin korrelaation lasku perustuu. Liitän tämän tiedon osaksi tuloksia. Jatkossa voi suodattaa epätarkimmat lajit halutessaan pois.
...
Pohjansirkun hyvä ennustustarkkuus (0.95) selittyy sillä, että siitä on havaintoja vain kolmelta reitiltä (3/264), muut ovat NA:ta. Ntype-analyysissähän tarkasteltiin, kuinka ennustustarkkuuteen vaikuttaa se, kuinka monta kertaa kukin reitti on laskettu. Tarkkuus varmaan vaihtelee kuitenkin myös sen mukaan, kuinka monelta reitiltä havainnot on kerätty. Sitähän me ei olla kontrolloitu laskuissa muuten kuin epäsuorasti prevalenssilla (0.05). Prevalenssi on kuitenkin laskettu koko aineistosta. Muutosta laskiessa huomioidaan vain periodien 1 ja 2 reitit (eli prevalenssi voi olla vielä alhaisempi näille aikajaksoille). Pa-aineistolla laskuihin päätyvät vähintään kahteen kertaan lasketut reitit (siis nuo samat 264 kpl) ja toisin kuin abu-aineistolla, jokaiselta laskennalta tulee lajille jokin arvo (siis joko 1 tai nolla). Pa-aineistolla korrelaatiot lasketaan siis 260:stä havainnosta ja mitään ongelmaa ei ole. Abu-aineistossa sen sijaan on paljon NA:ta eikä joistain lajeista siis kerry kuin kourallinen havaintoja. 

**Huom!** Muista, että abu-arvot ovat yksilömäärien logaritmeja.


### 1.4.6 Todelliset ja ennustetut muutokset tutkimusalueella
...
Hmm.. Jos näitä kuvia päädytään käyttämään, pitäisikö pohjansirkku tai peltopyy ottaa pois epätarkkoina? Ehkä ei? Koska havaintoja kyllä riittää mutta ne eivät vain satu samoille reiteille tai samoihin poolaus-ruutuihin.
...
Näistä kuvaajista on ehkäpä vaikea saada paljoa irti koska ennustustarkkuudet ovat niin huonoja yleisesti. Tässä myös muutos on laskettu koko alueelle. Eli vähentyminen toisaalla ja runsastuminen toisaalla saattavat kumota toisensa.
...
Tämäkään kuva ei varsinaisesti vihjaa siihen, että ennustustarkkuus riippuisi muutoksen määrästä.

HUOM! Näissä kuvissa on vielä mukana pohjansirkku ja peltopyy, vaikka myöhemmin poistan ne aineistosta. Siksi varmaankin alussa häntä todella korkealla.
