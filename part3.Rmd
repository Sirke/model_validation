...
Teen taulukon, jossa on lajikohtaiset ennustustarkkuudet.
...
Valitsen aineistosta vain metodi D:n, poolaamattomat ja poolatut ennustustarkkuudet, ja tarkastelen vain niitä vierekkäin.
...
Nyt aineistossa siis enää vain metodi D:n arvot.

### 3.3.3 Analyysi

Testataan vaikuttaako poolaus ja aineistotyyppi ennustustarkkuuksiin. Käytän samaa sekamallia kuin aikaisemminkin.(En siis vertaile random-rakenteita enää). Laji on random-muuttuja. Aineistona on kuitenkin vain poolaamattomat ja poolatut metodi D:llä lasketut ennustustarkkuudet (ei A,B,C). Onko tämä oikein?
...
Interaktio näyttäisi olevan merkitsevä.
...
Mallin validointi residuaaleilla:
...
Eli tässäkin on tuloksena, että poolauksen ja aineistotyypin yhdistelmä vaikuttavat ennustustarkkuuteen. Residuaalit ihan ok. Mutta erothan ovat ihan minimaalisia verrattuna muuhun aineistoon.

### verrataan C vs. pool_C
...
### Analyysi kaikilla poolatuilla metodeilla
...
Eli vain pa-aineistolla vain metodilla on väliä, ei niinkään poolauksella?
...
#the test
res.aov <- anova_test(
  data = res3, dv = accuracy, wid = species,
  within = c(pa, method)
  )
...
Sekä pa että abu aineistolla metodilla on väliä.
...
Sama tulos kuin yllä koska vain kaksi metodia vertailtavana.
...
Aineistotyypillä sen sijaan ei ole niin suurta vaikutusta. Oli aineisto poolattu tai ei, kummatkin aineistotyypit, sekä pa että abu, antavat yhtä "huonoja" ennustustarkkuuksia.

```{r}
#visualization
bxp <- ggboxplot(
  res3, x = "method", y = "accuracy",
...
Ehkä jokin ylläolevista kuvaajista olisi hyvä.
