...
Tarkistan laskut valitsemalla mukaan reitit kategorioista 3 ja 4 ja laskemalla laskut uusiksi. Tulokseksi pitäisi siis tulla perus metodi D:n mukaiset tulokset pa-aineistolle. Eli ~0.023.
```{r}
# COMPUTE MEASURES D
selroutes = routes[which(ntypes>=3)] #tässä siis kategoriat 3 ja 4 mukana!
nsr = length(selroutes)
...
Tulos on odotettu. Laskut ovat oikein.


## Abu-aineisto

Tehdään samat laskut abu-aineistolle:
...
Lasketaan kaikille kategorioille keskiarvot ja se:t
...
Keskiarvoissa ei näytä olevan suuria eroja. Vaihtelu suurinta ntype1 kategorioissa.

Video standard deviation ja standard error of the mean -aiheesta löytyy [täältä](https://www.youtube.com/watch?v=3UPYpOLeRJg).
...
Vertailu on sinänsä ongelmallinen, että vertaillaan eri määrää reittejä sekä eri paikoissa sijaitsevia reittejä keskenään. Tarkastetaan kartalta, missä kunkin kategorian reitit sijaitsevat.  
...
```{r}
n1=subset(xy,xy$ntypes==1)
ggplot(n1,aes(x=x,y=y))+geom_point()
```
Ntype3 reitit:
```{r}
n3=subset(xy,xy$ntypes==3)
ggplot(n3,aes(x=x,y=y))+geom_point()
```
Ntype4 reitit:
```{r}
n4=subset(xy,xy$ntypes==4)
ggplot(n4,aes(x=x,y=y))+geom_point()
```


Tosin reittien sijainnit eivät ole enää niin pahasti ryhmittyneitä kuin aiemmissa analyyseissä. Hieman ntype4 kategoria meinaa painottua Etelä-Ruotsiin. Suomi ei ole kovin hyvin edustettuna aineistossa.
